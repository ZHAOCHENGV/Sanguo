...
UChunk::BeginDestroy()
{
	...
	if (TraitsTemp)
	{
		for (int32 i = 0; i < TraitLinesNum(); ++i)
		{
			if (TraitsTemp[i])
			{
				...
				FMoreMemory::Free(TraitsTemp[i], 2);
			}
		}
		FMoreMemory::Free(TraitsTemp, 1);
	}
	...
	if (UNLIKELY(Mechanism))
	{
		...
		Mechanism->ChunksByTraitmarksCache.Remove(this->Traitmark);
		for (auto& Pair : Mechanism->ChunksByFilterCache)
		{
			if (Matches(Pair.Key))
			{
				Pair.Value.RemoveSwap(this);
			}
		}
	}
