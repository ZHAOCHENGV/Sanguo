...
struct APPARATUSRUNTIME_API FBeltSlotCache
{
  ...
  private:
	...
	Unlock()
	{
		...
		{
			if ((Details[i] != nullptr) && Details[i]->IsEnabled()) continue;
#if (ENGINE_MAJOR_VERSION == 5) && (ENGINE_MINOR_VERSION >= 4)
			Details.RemoveAtSwap(i--, 1, EAllowShrinking::No);
#else
  ...
  public:
	...
	OPTIONAL_FORCEINLINE FBeltSlotCache(const FBeltSlotCache& InCache)
	  : Owner(InCache.Owner)
	  , Details(InCache.Details)
	  , IterableCount(InCache.IterableCount)
	{}

	OPTIONAL_FORCEINLINE FBeltSlotCache(FBeltSlotCache&& InCache)
	  : Owner(InCache.Owner)
	...
	HasAny() const
	{
		...
		{
			if (LIKELY((Details[i] != nullptr) && Details[i]->IsEnabled()))
			{
				return true;
			}
		}
