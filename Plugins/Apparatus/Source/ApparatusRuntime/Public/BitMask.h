...
#pragma endregion Constants
	...
	struct FBitAccessor
	{
	  ...
	  public:
		...
		OPTIONAL_FORCEINLINE FBitAccessor&
  ...
  private:
	 ...
	 * indices.
	 */
	OPTIONAL_FORCEINLINE GroupType
	...
	{
		OPTIONAL_FORCEINLINE StaticConstructor()
		{
...
#pragma endregion Comparison
	...
	Trim(const bool bAllowShrinking = true) const
	{
		...
		if (RemovedCount > 0)
		{
			...
			MutableThis->Groups.RemoveAt(Groups.Num() - RemovedCount, RemovedCount, bAllowShrinking ? EAllowShrinking::Yes : EAllowShrinking::No);
#else
	 ...
	 * Move the bit mask.
	 */
	OPTIONAL_FORCEINLINE FBitMask& operator=(FBitMask&& InBitMask)
	{
		Groups = MoveTemp(InBitMask.Groups);
		return *this;
	}
	 ...
	 * Set this bit mask equal to another bit mask.
	 */
	OPTIONAL_FORCEINLINE FBitMask& operator=(const FBitMask& InBitMask)
	{
	...
	struct ConstIterator
	{
	  ...
	  public:
		...
		OPTIONAL_FORCEINLINE ConstIterator&
		operator++()
		{
			next();
			return *this;
	...
	OPTIONAL_FORCEINLINE Iterator end()
	{
		return Iterator(this, Iterator::InvalidIndex);
	}

	OPTIONAL_FORCEINLINE ConstIterator end() const
	{
...
#if (ENGINE_MAJOR_VERSION == 5) && (ENGINE_MINOR_VERSION >= 4)
			Groups.RemoveAt(InMask.Groups.Num(), Groups.Num() - InMask.Groups.Num(), EAllowShrinking::No);
#else
...
#pragma region Initialization
	 ...
	 */
	OPTIONAL_FORCEINLINE FBitMask(const FBitMask& InBitMask)
	  : Groups(InBitMask.Groups)
	{}
	 ...
	 */
	OPTIONAL_FORCEINLINE FBitMask(FBitMask&& InBitMask)
	  : Groups(MoveTemp(InBitMask.Groups))
	{}
