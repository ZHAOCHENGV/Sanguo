...
struct APPARATUSRUNTIME_API FScriptStructArray
{
  ...
  private:
	...
	/**
	 * Get the size of the structure, guaranteeing it be non-zero.
	 */
	...
	MemoryAt(const int32 Index)
	{
		...
		return (void*)((uint8*)Data + (SIZE_T)GetSafeStructureSize() * (SIZE_T)Index);
	}
	 ...
	 */
	OPTIONAL_FORCEINLINE const void*
	MemoryAt(const int32 Index) const
	{
		...
		check(ElementType);
		// Use SIZE_T cast to be able to address more memory:
		return (const void*)((const uint8*)Data + (SIZE_T)GetSafeStructureSize() * (SIZE_T)Index);
	}
  ...
  public:
	 ...
	 */
	OPTIONAL_FORCEINLINE UScriptStruct*
	...
	Reserve(const int32 InCapacity)
	{
		...
		check(ElementType);
		Capacity = InCapacity;
		FMoreMemory::ReallocateArray(Data, Capacity, GetSafeStructureSize(), ElementType->GetMinAlignment(), (uint16)((uint64)ElementType));
		return true;
	}
	    ...
	    const bool  bAllowShrinking = true)
	{
		...
		if (bAllowShrinking)
		{
			...
			if (Capacity != NewCapacity)
			{
				...
				FMoreMemory::ReallocateArray(Data, NewCapacity, GetSafeStructureSize(), ElementType->GetMinAlignment(), (uint16)((uint64)ElementType));
			}
		}
	}
		...
		const int32          InCapacity = 0)
	{
		...
		if (ElementType != InElementType)
		{
			...
			FMoreMemory::ReallocateArray(Data, InCapacity, GetSafeStructureSize(), ElementType->GetMinAlignment(), (uint16)((uint64)ElementType));
			Count = 0;
			Capacity = InCapacity;
	...
	Set(const FScriptStructArray& Array)
	{
		...
		if (ElementType != Array.ElementType)
		{
			...
			FMoreMemory::ReallocateArray(Data, Array.Capacity, GetSafeStructureSize(), ElementType->GetMinAlignment(), (uint16)((uint64)ElementType));
	  ...
	  , Capacity(InCapacity)
	{
		...
		check(Count >= 0);
		check(Capacity >= 0);
		FMoreMemory::AllocateArray(Data, Capacity, GetSafeStructureSize(), ElementType->GetMinAlignment(), (uint16)((uint64)ElementType));
	}
	...
	~FScriptStructArray()
	{
		...
		{
			ElementType->DestroyStruct(Data, Count);
			FMoreMemory::Free(Data, (uint16)((uint64)ElementType));
			Data = nullptr;
		}
	}
	 ...
	 */
	OPTIONAL_FORCEINLINE FScriptStructArray&
