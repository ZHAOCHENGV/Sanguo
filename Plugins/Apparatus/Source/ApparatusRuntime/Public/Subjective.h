...
#include "Engine/World.h" // Examining world's networking state.
...
OPTIONAL_FORCEINLINE FStandardSubjectivePropertyBlock*                                  \
...
OPTIONAL_FORCEINLINE bool IsNetworkCapable() const override                                    \
{                                                                                     \
	return true;                                                                      \
}                                                                                     \
OPTIONAL_FORCEINLINE FStandardSubjectivePropertyBlock*                                         \
...
OPTIONAL_FORCEINLINE FStandardSubjectiveNetworkPropertyBlock*                                  \
...
#pragma region Property Block
	...
	OPTIONAL_FORCEINLINE const FStandardSubjectivePropertyBlock&
...
#pragma region Network Property Block
	...
	OPTIONAL_FORCEINLINE const FStandardSubjectiveNetworkPropertyBlock&
...
#pragma region Direct Property Access
	 ...
	 * Constant version.
	 */
	OPTIONAL_FORCEINLINE const EFlagmark&
	 ...
	 * Get the internal flagmark property reference.
	 */
	OPTIONAL_FORCEINLINE EFlagmark&
	GetFlagmarkPropRef()
	{
		static_assert(sizeof(EFlagmark) == sizeof(int32), "Flagmark size must be that of a 32-bit integer.");
	 ...
	 * Mutable version.
	 */
	OPTIONAL_FORCEINLINE TArray<FTraitRecord>&
	GetTraitRecordsRef()
	{
	 ...
	 */
	OPTIONAL_FORCEINLINE TArray<UDetail*>&
	GetDetailsRef()
	{
...
#pragma endregion Direct Property Access
	 ...
	 */
	OPTIONAL_FORCEINLINE FTraitmark
	...
	OPTIONAL_FORCEINLINE const FTraitRecord*
	 ...
	 */
	virtual OPTIONAL_FORCEINLINE bool
  ...
  public:
	 ...
	 */
	OPTIONAL_FORCEINLINE const TArray<FTraitRecord>&
	 ...
	 */
	virtual OPTIONAL_FORCEINLINE const TArray<UDetail*>&
	GetDetailsRef() const
	{
		return GetStandardPropertyBlock().Details;
	}
...
#pragma region Traits
	...
	OPTIONAL_FORCEINLINE TOutcomeIf<Paradigm, IsTraitType<T>()>
	...
	template < EParadigm Paradigm, typename T,
			   TTraitTypeSecurity<T> = true >
	OPTIONAL_FORCEINLINE TOutcome<Paradigm, T>
	GetTrait() const
	{
		T TraitTemp;
...
#pragma region Direct Trait Data Access
	/// @name Direct Trait Data Access
	/// @{

	/**
	 * Get the immutable trait data from the subjective by its type.
	 * Dynamically-typed version.
	 * 
	 * If the trait is missing within the subjective,
	 * a @c nullptr is returned.
	 *
	 * @tparam Paradigm The paradigm to work under.
	 * @param TraitType The type of the trait to get.
	 * @return A pointer to the immutable trait's data.
	...
	/**
	 * Get the mutable trait data from a subjective by its type.
	 * 
	 * If the trait is missing within the subjective,
	 * a @c nullptr is returned.
	 *
	...
	/**
	 * Get immutable trait data from the subjective by its type.
	 * Statically-typed version.
	 * 
	 * If the trait is missing within the subject,
	 * a @c nullptr is returned.
	 ...
	 * @tparam T The type of the trait to get.
	 * @tparam Paradigm The paradigm to work under.
	 * @return A pointer to the mutable trait's data.
	 */
	template < typename T, EParadigm Paradigm = EParadigm::Default >
	OPTIONAL_FORCEINLINE auto
	GetTraitPtr()
	...
	/// @}
#pragma endregion Direct Trait Data Access
...
#pragma region Details
	...
	template < class D >
	[[deprecated("The 'FindDetail' method is deprecated for consistency reasons. Use the 'GetDetail' method instead, please.")]]
	OPTIONAL_FORCEINLINE typename std::enable_if<IsDetailClass<D>(), D*>::type
	...
	OPTIONAL_FORCEINLINE TOutcomeIf<Paradigm, IsDetailClass<D>(), D*>
	 ...
	 */
	template < class D >
	OPTIONAL_FORCEINLINE typename std::enable_if<IsDetailClass<D>(), bool>::type
	...
	OPTIONAL_FORCEINLINE TOutcome<Paradigm, D*>
	 ...
	 */
	template < EParadigm Paradigm, class D >
	OPTIONAL_FORCEINLINE TOutcomeIf<Paradigm, IsDetailClass<D>()>
...
#pragma endregion Networking
	 ...
	 */
	OPTIONAL_FORCEINLINE FSubjectHandle
	GetHandle()
	{
		if (UNLIKELY(!IsValidSubjective(this)))
		{
	 ...
	 * Constant version.
	 */
	OPTIONAL_FORCEINLINE FConstSubjectHandle
	GetHandle() const
	{
		if (UNLIKELY(!IsValidSubjective(this)))
		{
	 ...
	 */
	OPTIONAL_FORCEINLINE virtual uint32
	CalcHash() const 
	{
	 ...
	 * You should always call the base (default) subjective
	 * implementation of this method first in your override.
	 */
	OPTIONAL_FORCEINLINE virtual void
	 ...
	 */
	OPTIONAL_FORCEINLINE virtual AActor*
	GetActor() const
	{
		return nullptr;
	}
  ...
  : protected ISubjective
{
  ...
  private:
	 ...
	 * Direct access for the internal details array. Constant version.
	 */
	OPTIONAL_FORCEINLINE const TArray<UDetail*>&
	GetDetailsRef() const
	{
...
#pragma region Traits
	 ...
	 */
	template < EParadigm Paradigm = EParadigm::Default >
	OPTIONAL_FORCEINLINE TOutcome<Paradigm, const void*>
	GetTraitPtr(UScriptStruct* const TraitType) const
	{
	 ...
	 */
	template < EParadigm Paradigm = EParadigm::Default >
	OPTIONAL_FORCEINLINE TOutcome<Paradigm, void*>
	GetTraitPtr(UScriptStruct* const TraitType)
	{
		return ISubjective::template GetTraitPtr<MakeUnsafe(Paradigm)>(TraitType);
	}
	 ...
	 */
	template < EParadigm Paradigm, typename T >
	OPTIONAL_FORCEINLINE TOutcomeIf<Paradigm, IsTraitType<T>(), const T*>
	GetTraitPtr() const
	{
	 ...
	 */
	template < EParadigm Paradigm, typename T >
	OPTIONAL_FORCEINLINE TOutcomeIf<Paradigm, IsTraitType<T>(), T*>
	GetTraitPtr()
	{
...
#pragma region Details
	 ...
	 */
	template < class D >
	OPTIONAL_FORCEINLINE typename std::enable_if<IsDetailClass<D>(), void>::type
...
#pragma endregion Networking
	 ...
	 * Get the subject handle of this subjective.
	 */
	OPTIONAL_FORCEINLINE FSolidSubjectHandle
	GetHandle() const
	{
