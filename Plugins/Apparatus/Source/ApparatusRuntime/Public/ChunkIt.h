...
struct TChunkIt final
{
  ...
  public:
	 ...
	 */
	OPTIONAL_FORCEINLINE const FFilter&
	GetFilter() const
	{
		return Filter;
	}
	 ...
	 */
	OPTIONAL_FORCEINLINE SubjectHandleT
	GetSubject() const
	{
...
#pragma region Hinted Traits Getting
	...
	{
		checkf(IsViable(), TEXT("The chunk iterator is not valid to get a trait with hinting from. Is it at the end?"));
		checkf(Chunk->IsLocked(), TEXT("The iterator's chunk is not locked. Is the iterator used outside of its iteration loop?"));
