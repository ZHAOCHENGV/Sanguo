...
/**
 * Optionally forcing the inlining of a method.
 */
#define OPTIONAL_FORCEINLINE inline

#endif //#ifndef OPTIONAL_FORCEINLINE

#else //#ifdef DOXYGEN_ONLY

#ifndef OPTIONAL_FORCEINLINE
/**
 * Optionally forcing the inlining of a method.
 * 
 * You can define this prior to including Apparatus headers,
 * in order to use aggressive inlining of some methods.
 * 
 * @warning Forcing the inlining can result in some heavy compilation times
 * while providing little performance benefits, since the modern-day
 * compilers are quite smart when deciding on what to inline.
 * 
 * You can force the inlining on shipping builds like so (prior to any Apparatus includes):
 * @code{.cpp}
 * #if UE_BUILD_SHIPPING
 * #define OPTIONAL_FORCEINLINE OPTIONAL_FORCEINLINE
 * #endif
 * @endcode
 * 
 * Or inside of your project's @c .Build.cs file:
 * @code{.cs}
 * if (Target.Configuration == UnrealTargetConfiguration.Shipping)
 * {
 *     PublicDefinitions.Add("OPTIONAL_FORCEINLINE=OPTIONAL_FORCEINLINE");
 * }
 * @endcode
 */
#define OPTIONAL_FORCEINLINE inline
 ...
 */
constexpr OPTIONAL_FORCEINLINE EParadigm
 ...
 */
OPTIONAL_FORCEINLINE EParadigm&
