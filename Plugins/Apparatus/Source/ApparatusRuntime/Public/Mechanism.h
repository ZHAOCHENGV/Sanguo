  ...
  : public AActor
{
  ...
  private:
	...
	struct TFunctionInfo<R(Ts...)>
	{
		 ...
		 */
		static OPTIONAL_FORCEINLINE constexpr FilterType
		MakeFilter()
		{
			return FilterType();
		}
...
#pragma region Subjects
	 ...
	 */
	mutable TMap<FTraitmark, UChunk*> ChunksByTraitmarksCache;

	/**
	 * All of the chunks stored by their matching filters.
	 */
	mutable TMap<FFilter, TArray<UChunk*>> ChunksByFilterCache;
...
#pragma region Subjectives
	 ...
	 */
	TMap<UBelt::TagType, UBelt*> BeltByTag;

	/**
	 * All of the belts stored by their matching filters.
	 */
	mutable TMap<FFilter, TArray<UBelt*>> BeltsByFilterCache;
...
#pragma region Belts
	 ...
	 */
	template < EParadigm Paradigm = EParadigm::DefaultInternal >
	OPTIONAL_FORCEINLINE TOutcome<Paradigm, UBelt*>
	ObtainBeltByTag(const UBelt::TagType Tag)
	{
		...
		if (LIKELY((BeltPtr != nullptr) && (*BeltPtr != nullptr)))
		{
			return MakeOutcome<Paradigm, UBelt*>(EApparatusStatus::Noop, *BeltPtr);
		}
...
#pragma region Iterables
	...
	OPTIONAL_FORCEINLINE TArray<IterableT*>&
	GetIterablesRef();

	template <>
	OPTIONAL_FORCEINLINE TArray<UChunk*>&
	...
	template <>
	OPTIONAL_FORCEINLINE TArray<UBelt*>&
	...
	template < typename IterableT >
	OPTIONAL_FORCEINLINE TMap<FFilter, TArray<IterableT*>>&
	GetIterablesByFilterCacheRef();

	template <>
	OPTIONAL_FORCEINLINE TMap<FFilter, TArray<UChunk*>>&
	...
	template <>
	OPTIONAL_FORCEINLINE TMap<FFilter, TArray<UBelt*>>&
						  ...
						  TArray<IterableT*, TAllocator>& OutIterables)
	{
		...
		OutIterables = *MatchingIterables;
...
#pragma region Chains
	...
	OPTIONAL_FORCEINLINE TArray<TSharedRef<ChainT>>&
	GetChainsRef() const
	{
	 ...
	 */
	template <>
	OPTIONAL_FORCEINLINE TArray<TSharedRef<FChain>>&
	 ...
	 */
	template <>
	OPTIONAL_FORCEINLINE TArray<TSharedRef<FSolidChain>>&
...
#pragma endregion Concurrency
	 ...
	 */
	OPTIONAL_FORCEINLINE const TSet<ISubjective*>&
	 ...
	 */
	OPTIONAL_FORCEINLINE FAdjectiveInfo&
	...
	template < typename IterableT,
			   typename ChainT = FChain, typename FilterT = FFilter,
			   TFilterTypeSecurity<std::decay_t<FilterT>> = true >
	OPTIONAL_FORCEINLINE TSharedRef<ChainT>
	...
	OPTIONAL_FORCEINLINE TSharedRef<FSolidChain>
	...
	OPTIONAL_FORCEINLINE typename std::conditional<bInSolidity, TSharedRef<FSolidChain>, TSharedRef<FChain>>::type
				...
				const int32              Capacity = 32)
	{
		...
		{
			if (UNLIKELY(*CachedChunk == nullptr))
			{
				// Stale cache:
			...
			else
			{
				...
				return MakeOutcome<Paradigm, UChunk*>(EApparatusStatus::Noop, *CachedChunk);
			}
		}
...
#pragma region Spawning
	 ...
	 */
	template < EParadigm Paradigm = EParadigm::Default >
	OPTIONAL_FORCEINLINE TOutcome<Paradigm, FSubjectHandle>
