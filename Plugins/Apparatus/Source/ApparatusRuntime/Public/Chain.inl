...
FCommonChain::DecrementOperatingsCount() const
{
	...
	checkf(Owner != nullptr, TEXT("A chain must have an owning mechanism when the operating are being decremented."));
	// We have to save the current owner, since
	// it can get nullified due to a chain's destruction:
	...
	switch (OperatingsCount.fetch_sub(1, std::memory_order_acq_rel))
	{
		...
		case 1:
			...
			{
				// This can destroy a self-destructing chain:
				const_cast<FCommonChain*>(this)->Dispose();
			}
			break;
	}
	OwnerSave->DecrementOperatingsCount();
}
...
TChain<ChunkItType, BeltItType, Paradigm>::TRunnableMechanic<MechanicT, Ps...>::LaunchSync()
{
	...
	if (Super::Owner != nullptr)
	{
		Super::bStop.store(false, std::memory_order_relaxed);
		Super::Owner->IncrementOperatingsCount();
		if (Super::bConcurrent)
		{
			Super::Owner->IncrementConcurrencyLevel();
		}
		Super::DoBeginRun();
		DoRun();
		Super::DoEndRun();
