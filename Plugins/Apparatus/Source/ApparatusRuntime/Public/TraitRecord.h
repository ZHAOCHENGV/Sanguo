 ...
 */
USTRUCT(BlueprintType, Meta = (HasNativeMake = "/Script/ApparatusRuntime.ApparatusFunctionLibrary:MakeDefaultTraitRecord"))
struct APPARATUSRUNTIME_API FTraitRecord
{
  ...
  private:
	...
	static constexpr uint16 DebugMemoryTag = 4;
	...
	DoDestroyData(const bool bFree = true) const
	{
		...
		if (LIKELY(bFree))
		{
			FMoreMemory::Free(Data, DebugMemoryTag);
		}
	...
	DoCreateData() const
	{
		if (LIKELY(Type != nullptr))
		{
			...
			Type->RecursivelyPreload();
			FMoreMemory::Allocate(Data, FMath::Max(Type->GetStructureSize(), 1), Type->GetMinAlignment(), DebugMemoryTag);
	...
	DoRecreateData() const
	{
		...
		FMoreMemory::Reallocate(Data, FMath::Max(Type->GetStructureSize(), 1), Type->GetMinAlignment(), DebugMemoryTag);
		Type->InitializeStruct(Data);
#if WITH_EDITORONLY_DATA
		bForceEditorPull = true;
#endif
			...
			if (UNLIKELY(InType != nullptr))
			{
				if (UNLIKELY(InTraitRecord.Data == nullptr))
				{
					...
					if (LIKELY(Data != nullptr))
					{
						...
						FMoreMemory::Reallocate(Data, FMath::Max(InType->GetStructureSize(), 1), InType->GetMinAlignment(), DebugMemoryTag);
...
#pragma region Initialization
	 ...
	 */
	template < typename T >
	static OPTIONAL_FORCEINLINE FTraitRecord
...
#pragma region Assignment
	 ...
	 * @return Returns itself.
	 */
	OPTIONAL_FORCEINLINE TTraitRecord&
