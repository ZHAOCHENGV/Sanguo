 ...
 */
USTRUCT(BlueprintType, Meta = (HasNativeMake = "/Script/ApparatusRuntime.ApparatusFunctionLibrary:MakeTraitmark"))
struct APPARATUSRUNTIME_API FTraitmark
{
  ...
  public:
	 ...
	 */
	template <class T>
	OPTIONAL_FORCEINLINE static const FBitMask&
	GetTraitMask()
	{
	 ...
	 */
	OPTIONAL_FORCEINLINE explicit operator const TraitsType&() const
	{
		return Traits;
	}
...
#pragma region Comparison
	...
	CalcHash() const
	{
		return HashCombine(GetTypeHash(GetTraitsMask()), GetTypeHash(Traits.Num()));
	}
...
#pragma region Assignment
	...
	Set(const FTraitmark& InTraitmark)
	{
		...
		auto Outcome = GetTraitsMaskRef().template Set<MakePolite(Paradigm)>(InTraitmark.GetTraitsMask());
		if (LIKELY(IsEditorTransactionActive()
				|| (Outcome == EApparatusStatus::Success)
				|| (Traits.Num() != InTraitmark.Traits.Num())))
		{
...
#pragma region Initialization
	 ...
	 */
	template < typename... Ts, EParadigm Paradigm = EParadigm::Default >
	static OPTIONAL_FORCEINLINE FTraitmark
	Make()
	{
